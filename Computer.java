...
public class Computer {
    ...
    public Computer(){ //Constructor
        nearby = false; //starts with randomly guessing cells
        toGuess = new LinkedList<>();
        orientation = -1; //sets orientation to unknown
        boatCells = new LinkedList<>();
        multipleShips = false; //starts without multiple ship sinking enabled
    }
    ...
    public void turn(Board user) { //Computer takes its turn
        if (!nearby) { //a random cell should be guessed
            ...
            if (result.equals("Hit!")) { //if the cell contained a boat
                ...
                System.out.println("It was a miss."); //the cell the computer chose did not contain a boat
        } //end !nearby if
